...
class Functions implements FuncInterface {
  ...
  private setupAutoSave() {
    ...
    document.addEventListener('visibilitychange', async () => {
      if (document.hidden) {
        ...
        if (currentProfile) {
          try {
            await this.profiles.saveProfile(currentProfile);
            this.logger.createLog(`Auto-saved profile on visibility change: ${currentProfile}`);
          } catch (error) {
            console.warn('Failed to auto-save profile on visibility change:', error);
          }
        }
      }
    });
    ...
    setInterval(async () => {
      ...
      if (currentProfile) {
        try {
          await this.profiles.saveProfile(currentProfile);
          this.logger.createLog(`Auto-saved profile (periodic): ${currentProfile}`);
        } catch (error) {
  ...
  async profilesMenu(button: HTMLButtonElement) {
    ...
    const content = this.ui.createElement('div', {
      ...
      this.ui.createElement('div', {
        ...
        this.ui.createElement('h3', {
          style: 'color: var(--text); margin: 0; font-size: 16px; font-weight: 600;'
        }, ['Profile Manager']),
        ...
        this.ui.createElement('div', {
          ...
          this.ui.createElement('button', {
            class: 'flex items-center justify-center w-8 h-8 rounded-md hover:bg-opacity-80',
            style: 'background: var(--main); color: var(--bg-1); border: none;',
          ...
          this.ui.createElement('button', {
            ...
            style: 'color: var(--text); border: none; background: var(--white-05);',
            title: 'Profile Settings',
            onclick: async () => {
              const url = (await this.proto.processUrl("daydream://settings")) || "/internal/error/";
              const iframe = this.items.frameContainer!.querySelector("iframe.active") as HTMLIFrameElement | null;
      ...
      ...(currentProfile ? [this.ui.createElement('div', {
        class: 'p-4 border-b',
        style: 'border-bottom: 1px solid var(--white-10);'
      }, [
        this.ui.createElement('div', {
          ...
          this.ui.createElement('div', {
            ...
            this.ui.createElement('div', {
              ...
              style: 'background: var(--main); color: var(--bg-1); font-weight: 600;'
            }, [currentProfile.charAt(0).toUpperCase()]),
            
            this.ui.createElement('div', {}, [
              this.ui.createElement('div', {
                style: 'color: var(--text); font-size: 14px; font-weight: 500;'
              }, [currentProfile]),
              this.ui.createElement('div', {
      ...
      this.ui.createElement('div', {
        ...
        profilesList.length > 0 ? this.ui.createElement('div', {
          class: 'p-2'
        }, [
          this.ui.createElement('div', {
            style: 'color: var(--text); opacity: 0.7; font-size: 12px; font-weight: 500; margin-bottom: 8px; padding: 0 8px;'
          }, ['Available Profiles']),
          
          ...profilesList.map(profileId => 
            this.ui.createElement('div', {
              class: `profile-row flex items-center justify-between p-2 rounded-md cursor-pointer`,
              style: currentProfile === profileId 
                ? 'background: var(--main-20a); border: 1px solid var(--main-35a);'
                : 'border: 1px solid transparent;',
              onmouseenter: currentProfile !== profileId ? ((e: Event) => {
                (e.target as HTMLElement).style.background = 'var(--white-05)';
              }) : undefined,
              onmouseleave: currentProfile !== profileId ? ((e: Event) => {
                (e.target as HTMLElement).style.background = 'transparent';
              }) : undefined
            }, [
              this.ui.createElement('div', {
                class: 'flex items-center gap-3 flex-1',
                onclick: currentProfile !== profileId ? (() => this.switchToProfile(profileId)) : undefined
              }, [
                this.ui.createElement('div', {
                ...
                this.ui.createElement('div', {
                  ...
                  ...(currentProfile === profileId ? [this.ui.createElement('div', {
                    style: 'color: var(--main); font-size: 12px;'
                  }, ['● Active'])] : [])
                ])
              ]),
              ...
              this.ui.createElement('div', {
                ...
                ...(currentProfile !== profileId ? [this.ui.createElement('button', {
                  class: 'flex items-center justify-center w-5 h-5 rounded',
                  style: 'color: var(--success); border: none; background: transparent;',
          ...
          this.ui.createElement('i', {
            'data-lucide': 'users',
            style: 'width: 32px; height: 32px; color: var(--text); opacity: 0.4; margin: 0 auto 12px; display: block;'
          }, []),
          this.ui.createElement('div', {
            style: 'color: var(--text); font-size: 14px; font-weight: 500; margin-bottom: 4px;'
          }, ['No Profiles Yet']),
          this.ui.createElement('div', {
            style: 'color: var(--text); opacity: 0.7; font-size: 12px;'
      ...
      this.ui.createElement('div', {
        ...
        this.ui.createElement('div', {
          ...
          // Import button
          this.ui.createElement('button', {
            class: 'flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-md',
            style: 'color: var(--text); border: 1px solid var(--white-10); background: transparent; font-size: 12px;',
          ...
          this.ui.createElement('button', {
            class: 'flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-md',
            style: 'color: var(--text); border: none; background: var(--white-05); font-size: 12px;',
  ...
  async showCreateProfileDialog() {
    ...
    title.style.cssText = 'color: var(--text); font-size: 20px; font-weight: 600; margin: 0;';
    
    const closeBtn = document.createElement('button');
    closeBtn.className = 'flex items-center justify-center w-8 h-8 rounded-full hover:bg-opacity-80';
    closeBtn.style.cssText = 'color: var(--text); background: var(--white-05); border: none; transition: all 0.15s ease;';
    ...
    label.style.cssText = 'color: var(--text); font-size: 14px; font-weight: 500;';
    ...
    helpText.textContent = 'Choose a unique name for your profile. This will help you identify it later.';
    helpText.style.cssText = 'color: var(--text); opacity: 0.7; font-size: 12px;';
  ...
  async deleteProfile(profileId: string) {
    const confirmed = await this.showConfirm(`Are you sure you want to delete the profile "${profileId}"? This action cannot be undone.`, 'Delete Profile');
    if (confirmed) {
      try {
    ...
    fileInput.onchange = async (event: Event) => {
      ...
      if (file) {
        try {
          ...
          const profileName = await this.showPrompt("Enter a name for the imported profile:", profileData.profileId || "Imported Profile", "Import Profile");
  ...
  async clearCurrentProfileData() {
    const confirmed = await this.showConfirm("Are you sure you want to clear all current browsing data? This will clear cookies, localStorage, and IndexedDB data.", "Clear Data");
    if (confirmed) {
      try {
  ...
  private showModal(config: {
    ...
    return new Promise((resolve) => {
      const modal = this.ui.createElement('div', {
        class: 'fixed inset-0 z-50 flex items-center justify-center',
        style: 'background: rgba(0, 0, 0, 0.5); backdrop-filter: blur(2px);'
      }, [
        this.ui.createElement('div', {
          class: 'bg-white rounded-lg shadow-xl max-w-sm w-full mx-4 transform transition-all',
          style: `
            background: var(--bg-2);
            border: 1px solid var(--white-10);
          ...
          this.ui.createElement('div', {
            ...
            this.ui.createElement('div', {
              ...
              this.ui.createElement('div', {
                ...
                style: config.type === 'error' ? 'background: var(--error); color: white;' :
                       config.type === 'success' ? 'background: var(--success); color: white;' :
                       config.type === 'warning' ? 'background: var(--warning); color: white;' :
                       'background: var(--main); color: white;'
              }, [
                this.ui.createElement('i', {
                  'data-lucide': config.type === 'error' ? 'x-circle' :
                                config.type === 'success' ? 'check-circle' :
                                config.type === 'warning' ? 'alert-triangle' :
                                'info',
                  style: 'width: 16px; height: 16px;'
                }, [])
              ]),
          ...
          this.ui.createElement('div', {
            ...
            this.ui.createElement('button', {
              ...
              style: button.style || 'background: var(--main); color: var(--bg-1); border: none;',
              onclick: () => {
                closeModal();
                if (button.action) button.action();
                resolve(button.text.toLowerCase().includes('ok') || button.text.toLowerCase().includes('yes'));
              }
            }, [button.text])
          ) || [
            this.ui.createElement('button', {
  ...
  private showAlert(message: string, type: 'info' | 'success' | 'error' | 'warning' = 'info'): Promise<void> {
  ...
  private showConfirm(message: string, title: string = 'Confirm'): Promise<boolean> {
    return this.showModal({
      title,
      message,
      type: 'warning',
      buttons: [
        {text: 'Cancel', style: 'background: var(--white-10); color: var(--text); border: none;'},
        {text: 'Yes', style: 'background: var(--error); color: white; border: none;'}
      ]
    });
  }

  private showPrompt(message: string, defaultValue: string = '', title: string = 'Input'): Promise<string | null> {
    return new Promise((resolve) => {
      ...
      const modal = this.ui.createElement('div', {
        ...
        this.ui.createElement('div', {
          ...
          this.ui.createElement('div', {
            ...
            this.ui.createElement('p', {
              style: 'color: var(--text); margin: 0 0 12px 0; line-height: 1.5;'
            }, [message]),
          ...
          this.ui.createElement('div', {
            ...
            this.ui.createElement('button', {
              class: 'px-4 py-2 rounded-md font-medium',
              style: 'background: var(--white-10); color: var(--text); border: none;',
              onclick: () => {
                closeModal();
                resolve(null);
              }

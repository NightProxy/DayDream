...
export default async function routes(fastify: FastifyInstance) {
  ...
  fastify.get('/results/:query', async (request: FastifyRequest<{
    Params: { query: string }
  }>, reply: FastifyReply) => {
  ...
  fastify.get('/internal/icons/*', async (request: FastifyRequest<{
    Params: { '*': string }
  }>, reply: FastifyReply) => {
  ...
  fastify.setNotFoundHandler(async (request: FastifyRequest, reply: FastifyReply) => {
    return reply.status(404).sendFile('error/index.html', path.join(__dirname, 'dist/internal'));
  });
}
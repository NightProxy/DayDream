...
(async () => {
  try {
    sessionToken = await sessionStore.getItem<string>("token");
  } catch (e) {
    console.debug("[bare-mux] Failed to load session token:", e);
  }
})();

class AuthenticatedWebSocket extends NativeWebSocket {
  constructor(url: string | URL, protocols?: string | string[]) {
    let finalProtocols: string[] = Array.isArray(protocols)
      ? [...protocols]
      : protocols
        ? [protocols]
        : [];
    ...
    if (sessionToken) {
      try {
        const authData = JSON.stringify({ "Night-Auth": sessionToken });
        const authProtocol = btoa(authData);
        finalProtocols.push(authProtocol);
      } catch (e) {
        console.warn("[bare-mux] Failed to inject Night+ auth:", e);
      }
    }

    super(url, finalProtocols);
  }
}

globalThis.WebSocket = AuthenticatedWebSocket as any;
...
console.debug(
  `bare-mux: running v${(self as any).BARE_MUX_VERSION} (build ${(self as any).BARE_MUX_COMMITHASH})`,
);

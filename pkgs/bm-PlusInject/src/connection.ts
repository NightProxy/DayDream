...
async function searchForPort(): Promise<MessagePort> {
  // @ts-expect-error
  const clients: SWClient[] = await self.clients.matchAll({
    type: "window",
...
function tryGetPort(client: SWClient): Promise<MessagePort> {
  ...
  return new Promise((resolve) => {
    client.postMessage({ type: "getPort", port: channel.port2 }, [
      channel.port2,
    ]);
...
function testPort(port: MessagePort): Promise<void> {
  ...
  nativePostMessage.call(
    port,
    <WorkerRequest>{ message: { type: "ping" }, port: pingChannel.port2 },
    [pingChannel.port2],
  );
  return pingPromise;
}
  ...
  if (registerHandlers) {
    nativeServiceWorker.addEventListener("message", (event: MessageEvent) => {
      if (event.data.type === "getPort" && event.data.port) {
        ...
        nativePostMessage.call(event.data.port, newWorker.port, [
          newWorker.port,
        ]);
      }
    });
...
export class WorkerConnection {
  ...
  async sendMessage(
    ...
    nativePostMessage.call(
      this.port,
      <WorkerRequest>{ message: message, port: channel.port2 },
      toTransfer,
    );
